# Parley game + player image. One image, two entrypoints:
#   /bin/parley         - the game server (default)
#   /bin/parley-player  - the prompt-delivery player
...
WORKDIR /workspace/parley
COPY --from=build /workspace/parley/parley /bin/parley
COPY --from=build /workspace/parley/parley-player /bin/parley-player
COPY --from=build /workspace/parley/data ./data
COPY --from=build /workspace/parley/client ./client
